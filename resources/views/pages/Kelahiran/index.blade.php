            ...
            <div class="card-body">
                        ...
                        <form action="{{ route('peristiwa-kelahiran.index') }}" method="GET" class="row g-3">
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="No. Akta, tempat lahir...">
                                </div>
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" class="form-control"
                                       name="year" value="{{ request('year', date('Y')) }}"
                                       min="2000" max="{{ date('Y') }}">
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="sort" class="form-select">
                                    ...
                                    <option value="no_akta_asc" {{ request('sort') == 'no_akta_asc' ? 'selected' : '' }}>
                                        No. Akta A-Z
                                    </option>
                                    <option value="no_akta_desc" {{ request('sort') == 'no_akta_desc' ? 'selected' : '' }}>
                                        No. Akta Z-A
                                    </option>
                                </select>
                            </div>
                ...
                <div class="row mb-4">
                    ...
                    @php
                        ...
                        $currentYear = date('Y');
                        $thisYearCount = 0;
                        $withFiles = $kelahirans->where(function($kelahiran) {
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if($hasFiles)
                                        <div class="d-flex flex-column align-items-center">
                                            <span class="badge bg-info bg-opacity-25 text-info border border-info px-3 py-2"
                                                  data-bs-toggle="tooltip" title="{{ $kelahiran->media->count() }} dokumen">
                                                <i class="fas fa-file-alt me-1"></i> {{ $kelahiran->media->count() }}
                                            </span>
                                            <small class="text-muted mt-1">
                                                @foreach($kelahiran->media->take(2) as $media)
                                                    ...
                                                    @if(!$loop->last), @endif
                                                @endforeach
                                                @if($kelahiran->media->count() > 2) +{{ $kelahiran->media->count() - 2 }} @endif
                                            </small>
                                        </div>
                                    @else
                                        <span class="badge bg-secondary bg-opacity-25 text-secondary border border-secondary px-3 py-2"
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <small class="text-muted">{{ $kelahiran->created_at->format('d/m/Y') }}</small>
                                ...
                                <td class="text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="{{ route('peristiwa-kelahiran.show', $kelahiran->kelahiran_id) }}"
                                           ...
                                           data-bs-toggle="tooltip" title="Detail Kelahiran">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('peristiwa-kelahiran.edit', $kelahiran->kelahiran_id) }}"
                                           class="btn btn-sm btn-warning"
                                           data-bs-toggle="tooltip" title="Edit Kelahiran">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('peristiwa-kelahiran.destroy', $kelahiran->kelahiran_id) }}"
                                              method="POST" onsubmit="return confirmDelete('{{ $kelahiran->no_akta }}')"
                                              class="d-inline">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        <a href="{{ route('peristiwa-kelahiran.create') }}" class="btn btn-primary mt-3">
